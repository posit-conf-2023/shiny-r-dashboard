...
Open `exercises/ex02.Rmd`, which contains the code from the previous slide, and try running it knitting it.
...
* `theme` - specifies a theme to use for styling

* `navbar` - constructs a navigation bar at the top of the screen


## Your turn - Exercise 2 (again)

We've just seen a number of possible layout methods for a flexdashboard, lets return to the code in `exercises/ex02.Rmd` and try changing layout more purposefully this time (e.g. a column, multipage, or storyboard layout).
...
The values are then accessed via the read-only list called `params` within the report's R chunks.
...
:::
````{.markdown code-line-numbers="|6-7,15-16"}
---
...
:::
````{.markdown code-line-numbers="|6-12,20-21"}
---
...
This results in the document being served by shiny server (which has implications for sharing and publishing)
...
:::
````{.markdown code-line-numbers="|6|9-13|15-24|30-40|48-55|49,50,54|62-68|72-86"}
---
title: "Demo 04"
output: 
  flexdashboard::flex_dashboard:
...
`flexdashboard` provides to builtin html components that can be included in your dashboard:
...
:::
````{.markdown code-line-numbers="|47-57|59-69|71-87"}
---
...
This has nothing in particular to do with flexdashboard but is a super useful technique for improving interactivity.

With Shiny `plotOutput()`s can also be used to generate inputs based on user click events. Here we are using the brush rectangular selection to subset the data and then updating the relevant components.
...
:::
````{.markdown code-line-numbers="|44-53|16-30|25-30|60-82|84-94|89"}
---
